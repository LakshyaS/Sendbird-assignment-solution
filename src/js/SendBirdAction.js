...
let instance = null;
let customName = ''; // this will assign property value to group channels while creation
...
class SendBirdAction {
    ...
    return new Promise((resolve, reject) => {
      if (this.userQuery.hasNext && !this.userQuery.isLoading) {
        this.userQuery.next((list, error) => {
          //alert(JSON.stringify(list)); //mychanges
  ...
  createOpenChannel(channelName) { //mychange - need to see if this gets called at all
    return new Promise((resolve, reject) => {
      channelName
        ?
  ...
  getGroupChannelList(isInit = false) {
    if (isInit || isNull(this.groupChannelQuery)) {
      ...
      this.groupChannelQuery.customTypeFilter = customName;
    }
    return new Promise((resolve, reject) => {
  ...
  createGroupChannel(userIds) { // THIS IS CALLED WHEN YoU CREATE NEW GROUP CHANNEL. aFTER SELECTING USERS, ON HITTING CREATE BUTTON 
    return new Promise((resolve, reject) => {
      let params = new this.sb.GroupChannelParams();
      params.customType = customName;
