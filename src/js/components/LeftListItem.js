...
let leftcustfilter = '';  // filter criteria
...
class LeftListItem {
  ...
  get title() {
    ...
    return this.channel.isOpenChannel()
      ? `# ${this.channel.name}`
      : (this.channel.customType===leftcustfilter?
        this.channel.members
          .map(member => {
           // alert(protectFromXSS(member.nickname));
            //UserList.getInstance().setCustomType()
            return protectFromXSS(this.channel.customType + member.nickname);
          })
          .join(', ')
          :''
          );
  ...
  _createElement(handler) {
    ...
    if (this.channel.isOpenChannel()) {
      ...
      if(this.channel.customType === leftcustfilter){
      const itemTop = createDivEl({ className: styles['item-top'] });
      const itemTopCount = createDivEl({ className: styles['item-count'], content: this.memberCount });

      const itemTopTitle = createDivEl({ className: styles['item-title'], content: this.title }); //my - this is shown on left
      //alert(this.title);
